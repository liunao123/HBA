...
// ! 关键帧参数（默认值，可通过 config.yaml key_frame 节点覆盖）
float THRESH_DISTENT = 5.0;
float THRESH_YAW_DEGREE = 10.0;

std::vector<std::string> collect_sorted_bag_files(const std::string &bag_path_or_dir)
{
  namespace fs = std::filesystem;
  fs::path input_path(bag_path_or_dir);
  std::vector<fs::path> bag_paths;

  if (fs::is_regular_file(input_path))
  {
    bag_paths.push_back(input_path);
  }
  else if (fs::is_directory(input_path))
  {
    for (const auto &entry : fs::directory_iterator(input_path))
    {
      ...
      if (p.extension() == ".bag")
      {
        bag_paths.push_back(p);
      }
    }
  }

  std::sort(bag_paths.begin(), bag_paths.end(), [](const fs::path &a, const fs::path &b)
            {
    const std::string a_name = a.filename().string();
    const std::string b_name = b.filename().string();
    if (a_name == b_name) return a.string() < b.string();
    return a_name < b_name; });

  std::vector<std::string> result;
  result.reserve(bag_paths.size());
...
void write_odom_yaml(double timestamp, const GnssOdomData &g, int id)
{
  ...
  node["pose"].push_back(g.pose(0, 0));
  node["pose"].push_back(g.pose(0, 1));
  node["pose"].push_back(g.pose(0, 2));
  node["pose"].push_back(g.pose(1, 0));
  node["pose"].push_back(g.pose(1, 1));
  node["pose"].push_back(g.pose(1, 2));
  node["pose"].push_back(g.pose(2, 0));
  node["pose"].push_back(g.pose(2, 1));
  node["pose"].push_back(g.pose(2, 2));
    ...
    double frame_time)
{
  ...
  for (const auto &pt : raw_cloud)
  {
    ...
    auto it = std::upper_bound(enu_poses.begin(), enu_poses.end(), t,
                               [](double v, const TumPose &tp)
                               { return v < tp.timestamp; });
...
int main(int argc, char **argv)
{
  ...
  // 从 yaml 读取关键帧提取参数
  if (config["key_frame"]) {
    THRESH_DISTENT    = config["key_frame"]["thresh_distance"].as<float>(THRESH_DISTENT);
    THRESH_YAW_DEGREE = config["key_frame"]["thresh_yaw_degree"].as<float>(THRESH_YAW_DEGREE);
  }
  std::cout << "[KeyFrame] thresh_distance=" << THRESH_DISTENT
            << " m, thresh_yaw=" << THRESH_YAW_DEGREE << " deg" << std::endl;
  // return 1;

  // 读取lidar_gnss_extrinsic外参（从calib文件夹）
  ...
  Eigen::Vector3d t_extr(0, 0, 0);
  {
    ...
    if (tf_cache.lookupTransform("cgi830", "hesai128", T_gnss_lidar)) {
      ...
      std::cerr << "[Extrinsic] Failed to lookup lidar to  ins, using identity!" << std::endl;
    }
  }
  std::cout << "[Extrinsic] ===========================" << std::endl;
  std::cout << "[Extrinsic] OK to lookup lidar to  ins " << std::endl;
  std::cout << q_extr << std::endl;
  std::cout << t_extr << std::endl;
  std::cout << "[Extrinsic] ===========================" << std::endl;
  ...
  std::vector<std::string> bag_files = collect_sorted_bag_files(bag_path);
  if (bag_files.empty())
  {
    std::cerr << "No bag files found from bag_path: " << bag_path << std::endl;
    return 1;
  }

  std::cout << "Found " << bag_files.size() << " bag file(s), sorted by filename:" << std::endl;
  for (size_t i = 0; i < bag_files.size(); ++i)
  {
    std::cout << "  [" << i + 1 << "] " << bag_files[i] << std::endl;
  }
  ...
  int count_gnss = 0;
  for (const auto &one_bag_path : bag_files)
  {
    std::cout << "Processing bag #" << one_bag_path << "\n" << std::endl;
    rosbag::Bag bag;
    try
    {
      bag.open(one_bag_path, rosbag::bagmode::Read);
    }
    catch (const std::exception &e)
    {
      ROS_ERROR("Failed to open bag %s: %s", one_bag_path.c_str(), e.what());
      continue;
    }
  ...
  for (const auto &one_bag_path : bag_files)
  {
    ...
    try
    {
      bag2.open(one_bag_path, rosbag::bagmode::Read);
    }
    catch (const std::exception &e)
    {
      ROS_ERROR("Failed to open bag %s: %s", one_bag_path.c_str(), e.what());
