 ...
 * @param output_image_path 投影结果图像的保存路径
 * @param colored_pcd_path 彩色点云的保存路径
 * @return 成功返回true，失败返回false
 */
bool project_points_to_image(
    const cv::Mat& image,
    const cv::Mat& K,
    const cv::Mat& D,
    ...
    const std::string& colored_pcd_path)
{
    if (image.empty()) {
    ...
    if (!cv::imwrite(output_image_path, overlay)) {
        std::cerr << "[project_points_to_image] Failed to save image to: " << output_image_path << std::endl;
        return false;
    }
    std::cout << "[project_points_to_image] Saved projection image to: " << output_image_path << std::endl;

    // 8. 保存彩色点云
    colored_cloud->width = static_cast<uint32_t>(colored_cloud->size());
    colored_cloud->height = 1;
    colored_cloud->is_dense = false;
    if (pcl::io::savePCDFileBinary(colored_pcd_path, *colored_cloud) == -1) {
        std::cerr << "[project_points_to_image] Failed to save colored PCD to: " << colored_pcd_path << std::endl;
        return false;
    }
    std::cout << "[project_points_to_image] Saved colored point cloud to: " << colored_pcd_path << std::endl;
...
int main(int argc, char **argv)
{
    ...
    for (const auto &frame : frame_timestamps)
    {
        ...
        for (int cam_idx = 1; cam_idx <= 7; ++cam_idx)
        {
            ...
            if (!project_points_to_image(image, K, D, T_cam_lidar, cloud, output_image, colored_pcd))
            {
